...
import com.keeps.manage.utils.CreateAdvCacheFile;
...
public class AdvPositionServiceImpl extends AbstractService implements AdvPositionService {
	...
	public String cacheAdvFile(String ids){
		...
		String filepath = Constants.file_upload_path+File.separator+Constants.ADV_FILE_PATH;
		for (String id : idarr) {
			...
			if (listadv.size()==0) {//没有广告，把广告位生成默认的
				...
				case 1://广告位类别是图片
					CreateAdvCacheFile.createManyImageAdvPosition(filepath, filename, advPosition);
					break;
				case 2://广告位类别是文字
					break;
				case 3://广告位类别是幻灯
					CreateAdvCacheFile.createSlideshowAdvPosition(filepath, filename,advPosition);
					break;
				case 4://广告位类别是Flash
					break;
				}
				...
				switch (advPosition.getApClass()) {
				case 1://广告位类别是图片
					CreateAdvCacheFile.createManyImageAdv(filepath, filename, listadv);
					break;
				case 2://广告位类别是文字
					break;
				case 3://广告位类别是幻灯
					CreateAdvCacheFile.createSlideshowAdv(filepath, filename,listadv);
					break;
				case 4://广告位类别是Flash
					break;
				}
