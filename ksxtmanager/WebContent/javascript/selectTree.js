(function($) {
	$.fn.selectTotree = function(options) {
		var defaults = {
			language: "zh-CN",
			ispselect: false,
			isshrink: false
		...
		opts.templateResult = function(data, container) {
			if(data.element) {
				...
				if($element.attr("parent") && $element.attr("parent")!=0) {
	...
	function open() {
		setTimeout(function() {
			...
			$(".select2-results__options li").each(function() {
				...
				if($this.attr("parent")) {
					$(this).siblings("li[val=" + $this.attr("parent") + "]").find("span:eq(0)").addClass("glyphicon glyphicon-chevron-down").css({
						"padding": "0 10px",
						"cursor": "default"
					});
