  $(function () {
       ...
       //ie8下不支持placeholder
        if (!("placeholder" in document.createElement("input"))) {
         ...
         //点击显示菜单
         $(".g-nav").on("click","li a", function(e) {
            $(".g-nav .J_tip").tooltip("hide"); 
            $(this).parent("li").toggleClass("active").children("ul").toggle();
            $(this).parent("li").siblings().removeClass("active").children("ul").hide();
            $(".g-nav .nav-second-level").find(".gi-23").removeClass("gi-24");
            if($(this).parent("li").hasClass("active")){
                //此处代表是二级菜单
                if($(this).has(".gi-23")){
                     $(".g-nav .nav-third-level li").removeClass("active");
                     $(this).find(".gi-23").addClass("gi-24");
                }
            }
            //一级菜单
            if($(this).parent().parent().hasClass("nav-first-level")){
                 $(".g-nav .nav-first-level").find(".nav-third-level").hide();
                 $(".g-nav .nav-first-level").find("ul li").removeClass("active");
            }
        });
        //主菜单、迷你菜单转换
        ...
        /*头部菜单*/
       /* if (window.PIE) {
            PIE.attach($('.J_adminAction').find("em")[0]);
        }*/
