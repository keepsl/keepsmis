...
public class NewsDaoImpl extends AbstractDao implements NewsDao {
	...
	public Page query(TNews news) {
		...
		if(StringUtils.hasText(news.getSidx()) && StringUtils.hasText(news.getSord())) {
			if ("empname".equals(news.getSidx())) {
				sb.append("order by c.name "+news.getSord());
			}else if("dicttypename".equals(news.getSidx())){
				sb.append("order by b.name "+news.getSord());
			}else{
				sb.append("order by a."+news.getSidx()+" "+news.getSord());
			}
		}else{
