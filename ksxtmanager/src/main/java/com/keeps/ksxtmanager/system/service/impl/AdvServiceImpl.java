...
import com.keeps.ksxtmanager.system.dao.AdvDao;
import com.keeps.ksxtmanager.system.service.AdvService;
import com.keeps.ksxtmanager.utils.aliyunoss.AliyunOSSClientUtil;
	...
	public String saveOrUpdate(TAdv adv, MultipartFile advcontentfile, HttpServletRequest request) {
		...
		if (adv.getApClass()==2) {//文字广告位
			...
			if (advcontentfile==null) {//修改时可空
				...
				/** start **/
				String origfilename = advcontentfile.getOriginalFilename();
				String filetype = origfilename.substring(origfilename.lastIndexOf(StringUtils.Symbol.POINT)+1);
				if(!CommonUtils.isExistStr(filetype.toLowerCase(), new String[]{"jpg","bmp","gif","png"}))
					throw new CapecException("不允许上传" + filetype + "格式的文件！");
				String filename = UUID.randomUUID() +"."+ filetype;
				Assert.isTrue(advcontentfile.getSize() / (1024 * 1024) <= Constants.MAX_UPLOAD_IMAGE_SIZE,"上传的图片大小请控制在" + Constants.MAX_UPLOAD_IMAGE_SIZE + "M内!");
				try {
					AliyunOSSClientUtil.uploadObject2OSS(advcontentfile.getInputStream(), Constants.ADV_IMAGE_PATH, filename);
				} catch (IOException e) {
					log.error(e);
					throw new CapecException("文件上传失败!");
				}
				...
				if (StringUtils.hasText(adv.getAdvContentpath())) {
					AliyunOSSClientUtil.deleteFile(Constants.ADV_IMAGE_PATH, adv.getAdvContentpath());
