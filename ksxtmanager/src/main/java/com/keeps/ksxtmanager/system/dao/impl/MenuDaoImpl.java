...
public class MenuDaoImpl extends AbstractDao implements MenuDao {
	...
	public Page queryList(TManagerMenu menu) {
		...
		if(StringUtils.hasText(menu.getSidx()) && StringUtils.hasText(menu.getSord())) {
			hql.append("order by "+menu.getSidx()+" "+menu.getSord());
		}else{
	...
	public List<TreeNode> queryListTreeAll(){
		StringBuffer sb = new StringBuffer();
		sb.append(" select id,pid as pId,url,menuname as name,'1' as type from t_manager_menu ");
		List<Object> values = new ArrayList<Object>();
