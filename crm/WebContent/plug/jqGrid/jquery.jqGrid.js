...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		updatepager = function(rn, dnd) {
			...
			if(pgboxes) {
				...
				if(ts.p.pginput===true) {
					$("#input"+tspg).html($.jgrid.template($.jgrid.getRegional(ts, "defaults.pgtext", ts.p.pgtext) || "","<input "+getstyle(stylemodule, 'pgInput', false, 'ui-pg-input') + " type='text' size='2' onkeyup='if(event.keyCode!=37 && event.keyCode != 39) value=value.replace(/\\D/g,\"\");if(value>"+last+") value=1;' onblur='if(value==\"\") value=1;if(value==0) value=1' maxlength='7' value='1' role='textbox'/>","<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>"));
					if(ts.p.toppager) {
						$("#input_t"+tspg_t).html($.jgrid.template($.jgrid.getRegional(ts, "defaults.pgtext", ts.p.pgtext) || "","<input "+getstyle(stylemodule, 'pgInput', false, 'ui-pg-input') + " type='text' onkeyup='if(event.keyCode!=37 && event.keyCode != 39) value=value.replace(/\\D/g,\"\");if(value>"+last+") value=1;' onblur='if(value==\"\") value=1;if(value==0) value=1' size='2' maxlength='7' value='1' role='textbox'/>","<span id='sp_1_"+$.jgrid.jqID(pgid)+"_toppager'></span>"));
					}
		...
		setPager = function (pgid, tp){
			...
			if(dir==="rtl") { pgl += str; }
			if(ts.p.pginput===true) { 
				pginp= "<td id='input"+tp+"' dir='"+dir+"'>"+$.jgrid.template( $.jgrid.getRegional(ts, "defaults.pgtext", ts.p.pgtext) || "","<input class='ui-pg-input' type='text' size='2' maxlength='7' value='1' role='textbox'/>","<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>")+"</td>";
			}
			if(ts.p.pgbuttons===true) {
